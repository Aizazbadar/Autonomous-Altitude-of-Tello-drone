...
Here, we can clearly see that the drone becomes unstable due to time delay, computer vision processing, image processing latency, Wi-Fi communication delay, noisy visual feedback, and the slow response of the control loop. and drone is repeatedly trying to reach the target
...
1. A visual target is detected in the camera stream using YOLO.
2. The target’s pixel position is compared with a reference position.
3. The error is used by the controller to compute a control signal.
4. The command is transmitted to the drone through Wi-Fi using the Tello SDK.
5. The drone adjusts its altitude accordingly.
6. The process repeats continuously in closed loop.
...
- Ryze Tello drone
- Laptop or PC for off-board processing
- Indoor test setup
